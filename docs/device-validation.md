...
- recent notification publication and alert-open records.
...
- the selected briefing-window start and observed publication offset; and
- the first time the user opened the app from that notification, when available.
...
notification promptly or that the user saw it. An open record proves that the
notification content intent was used, but its publication-to-open offset
includes both operating-system delay and human response time; it cannot separate
those components.

The notification carries a private random audit identifier so the tap can be
matched to its local publication record. The identifier is never included in
the shared report. Older publication records without an identifier remain
readable but cannot gain open evidence. Repeated taps preserve the first open
time.
...
5. Open a received alert from the Android notification surface. The app records
   the first tap automatically; dismissing an alert creates no open evidence.
6. Select `Refresh evidence`, then use `Share report` to review publication and
   open offsets.
...
No notification-listener permission is requested. The app learns about an open
only from its own notification content intent and does not inspect notifications
from any other app.
